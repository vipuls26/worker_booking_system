...
<template>
    <section class="rounded-lg bg-white shadow-sm ring-1 ring-gray-200 dark:bg-gray-900 dark:ring-white/10">
        ...
        <div v-else class="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">
            <article v-for="day in groupedDays" :key="day.value" class="rounded-lg border border-gray-200 p-4 dark:border-white/10">
                ...
                <div v-if="day.offDay" class="mt-4 flex items-center justify-between gap-3 rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-800 dark:bg-amber-500/10 dark:text-amber-200">
                    ...
                    <div class="flex gap-1">
                        <button type="button" class="inline-flex size-8 items-center justify-center rounded-md hover:bg-amber-100 dark:hover:bg-amber-500/20" @click="$emit('edit', day.offDay)">
                            <i class="pi pi-pencil text-xs" aria-hidden="true"></i>
                        </button>
                        <button type="button" class="inline-flex size-8 items-center justify-center rounded-md text-red-600 hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-500/10" @click="$emit('delete', day.offDay)">
                ...
                <div v-else-if="day.windows.length" class="mt-4 space-y-2">
                    <div v-for="schedule in day.windows" :key="schedule.id" class="flex items-center justify-between gap-3 rounded-md bg-gray-50 px-3 py-2 dark:bg-gray-950">
                        <span class="text-sm font-medium text-gray-800 dark:text-gray-100">{{ schedule.start_time }} - {{ schedule.end_time }}</span>
                        <div class="flex gap-1">
                            <button type="button" class="inline-flex size-8 items-center justify-center rounded-md text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-white/10" @click="$emit('edit', schedule)">
                                <i class="pi pi-pencil text-xs" aria-hidden="true"></i>
                            </button>
                            <button type="button" class="inline-flex size-8 items-center justify-center rounded-md text-red-600 hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-500/10" @click="$emit('delete', schedule)">
                                <i class="pi pi-trash text-xs" aria-hidden="true"></i>
                            </button>
                        </div>
                    </div>
                ...
                <div v-else class="mt-4 rounded-md border border-dashed border-gray-300 p-3 text-center dark:border-white/10">
                    ...
                    <div class="mt-3 flex flex-col justify-center gap-2 min-[360px]:flex-row">
                        <AppButton type="button" icon="pi-plus" size="sm" :full-width="false" @click="$emit('add-day', day.value)">
                            Add hours
                        </AppButton>
                        <AppButton type="button" icon="pi-ban" size="sm" variant="secondary" :full-width="false" @click="$emit('mark-off-day', day.value)">
                            Mark off
                        </AppButton>
                    </div>
                </div>
