        ...
        <section class="w-full max-w-lg rounded-lg bg-white shadow-xl ring-1 ring-gray-200 dark:bg-gray-900 dark:ring-white/10">
            ...
            <form class="space-y-4 p-5" @submit.prevent="submit">
                ...
                <label class="flex items-center justify-between gap-4 rounded-md border border-gray-200 px-3 py-2 text-sm text-gray-700 dark:border-white/10 dark:text-gray-200">
                    ...
                    <input v-model="form.is_off_day" type="checkbox" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900 dark:border-white/10 dark:bg-gray-950 dark:focus:ring-white">
                </label>

                <div v-if="!form.is_off_day" class="grid gap-4 sm:grid-cols-2">
                    <FormInput id="schedule_start" v-model="form.start_time" label="Start time" type="time" :error="errors.start_time" />
                    <FormInput id="schedule_end" v-model="form.end_time" label="End time" type="time" :error="errors.end_time" />
                </div>

                <div class="flex justify-end gap-2 pt-2">
                    <AppButton type="button" variant="secondary" :full-width="false" @click="$emit('close')">Cancel</AppButton>
                    <div class="w-auto">
                        <AppButton type="submit" icon="pi-save" :loading="loading">{{ loading ? 'Saving...' : 'Save availability' }}</AppButton>
                    </div>
                </div>
            </form>
        </section>
