    ...
    <DashboardLayout title="Manage Bookings">
        <div class="space-y-5">
            ...
            <div v-else class="space-y-4">
                <article v-for="booking in bookingsStore.bookings" :key="booking.id" class="rounded-lg bg-white p-5 shadow-sm ring-1 ring-gray-200 dark:bg-gray-900 dark:ring-white/10">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div class="flex flex-col gap-2 sm:w-72">
                            ...
                            <div v-for="action in actionsFor(booking.status)" :key="action.status">
                                ...
                                <AppButton v-else :icon="action.icon" :loading="bookingsStore.saving" @click="updateStatus(booking, action.status)">
                                    {{ action.label }}
                                </AppButton>
                            </div>
                    ...
                    <div v-if="booking.review || booking.worker_review || reviewingBookingId === booking.id" class="mt-5 grid gap-4 lg:grid-cols-2">
                        ...
                        <div class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-white/10 dark:bg-gray-950">
                            ...
                            <form v-else-if="reviewingBookingId === booking.id" class="mt-3 space-y-3" @submit.prevent="submitCustomerReview(booking)">
                                ...
                                ></textarea>
                                <AppButton type="submit" icon="pi-star" :loading="bookingsStore.saving" :disabled="reviewForm.rating === 0">Submit feedback</AppButton>
                            </form>
